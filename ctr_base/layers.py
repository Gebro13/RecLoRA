...
class Embeddings(nn.Module):
    def __init__(self, config: Config):
        super().__init__()
        self.embedding = nn.Embedding(config.num_features, embedding_dim=config.embed_size)
...
class MLPBlock(nn.Module):
    def __init__(self, input_dim, hidden_size=128, num_hidden_layers=3,
        ...
        if not isinstance(hidden_act, list):
            hidden_action = [get_act(hidden_act) for i in range(num_hidden_layers)]
        for i in range(num_hidden_layers):
            dense_layers.append(nn.Linear(input_dim, hidden_size))
            if batch_norm:
                pass
            dense_layers.append(hidden_action[i])
...
#         if mask is not None:
#             attention = attention.masked_fill_(mask.float() == 0, -np.inf)
...
class MultiHeadSelfAttention(MultiHeadAttention):
    def forward(self, X, mask=None):
        output, attention = super(MultiHeadSelfAttention, self).forward(X, X, X, mask)
        return output


class DIN_Attention(nn.Module):
    def __init__(self, config: Config, use_softmax=False):
        super(DIN_Attention, self).__init__()
        self.embed_size = config.embed_size
        self.use_softmax = use_softmax
        hidden_act = config.hidden_act
        if isinstance(config.hidden_act, str) and config.hidden_act.lower() == "dice":
            hidden_act = [Dice(config.hidden_size) for _ in range(config.num_attn_layers)]
        self.attention_layer = nn.Sequential(*[
            MLPBlock(
                input_dim=config.embed_size * 4, 
                hidden_size=config.hidden_size, 
                num_hidden_layers=config.num_attn_layers, 
    ...
    def forward(self, target_item, hist_embed, hist_mask):
        """
        :param target_item: (B, embed)
        :param history_sequence: (B, len, embed)
        :param mask: mask of history_sequence, 0 for masked positions
        """
        hist_len = hist_embed.shape[1]
        target_item = target_item.unsqueeze(1).expand(-1, hist_len, -1)
        attention_input = torch.cat([
            target_item, hist_embed,
            target_item - hist_embed,
            target_item * hist_embed,], 
            dim=-1
        )

        attention_weight = self.attention_layer(attention_input.view(-1, 4 * self.embed_size))
        attention_weight = hist_mask.float() * attention_weight.view(-1, hist_len)
        if self.use_softmax:
            attention_weight -= torch.max(attention_weight, dim=-1, keepdim=True)
            attention_weight -= 1.e9 * (1 - hist_mask.float())
            attention_weight = F.softmax(attention_weight, dim=-1)
        output = (attention_weight.unsqueeze(-1) * hist_embed).sum(dim=1)
        return output
    

class AGRUCell(nn.Module):
    """
    AGRUCell w/ attentional update gate
    """
...
class DynamicGRU(nn.Module):
    ...
    def  __init__(self, input_size, hidden_size, bias=True, gru_type="AUGRU"):
        ...
        if gru_type == "AUGRU":
            ...
            self.gru_type = AGRUCell(input_size, hidden_size, bias)
